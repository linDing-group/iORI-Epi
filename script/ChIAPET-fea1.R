...
fileAnnot=list.files("data/ChIA-PET/MCF7/")
...
for(i in 1:length(AnnotNames)){
    GenomicFeatureList.GR[[i]] <- sort(unique(readGFBed(paste0("data/ChIA-PET/MCF7/",fileAnnot[i]),SeqInfo)))
    print(paste0(AnnotNames[i]," : ",length(GenomicFeatureList.GR[[i]])))

names(GenomicFeatureList.GR)=AnnotNames
GenomicFeatureList.GR=GenomicFeatureList.GR[names(GenomicFeatureList.GR)%in%c("ChIA-PET1")]
