...
fileAnnot=list.files("../data/test_Epi")
...
for(i in 1:length(AnnotNames)){
    GenomicFeatureList.GR[[i]] <- sort(unique(readGFBed(paste0("../data/test_Epi/",fileAnnot[i]),SeqInfo)))
...
filename = "test"
file_varimp= paste(filename,"Epimarks_Fea.csv")
